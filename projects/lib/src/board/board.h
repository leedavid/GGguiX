...
class LIB_EXPORT Board
{
	...
	public:
		...
		virtual QString moveToSan(const Move& move) = 0;
		...
		int plyCount() const;
		// Move number in game (incremented after each black move)
...
inline int Board::moveNumber() const
{	
	if (m_side == Side::Black) {
		return m_moveHistory.size() / 2;
	}
	else {
		return m_moveHistory.size() / 2 +1;
	}
}
