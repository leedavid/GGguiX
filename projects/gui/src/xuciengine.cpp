...
namespace Chess {
    ...
    bool UCIEngine::startAnalysis(const Board& board, int nv, const EngineParameter& mt, bool bNewGame, QString line)
    {
        ...
        if (line.isEmpty())
        {
            m_position = board.fenString();
            //m_position = board.toFen(m_chess960);
        }
        else
        {
            m_position = m_startPos->fenString();
        ...
        while (!info.section(' ', section, section + 1, QString::SectionSkipEmpty).isEmpty())
        {
            ...
            if (name == "score")
            {
                ...
                if (type == "cp" || type == "mate")
                {
                    ...
                    else if (m_board->sideToMove() == Side::Black)
                    {
            ...
            if (name == "pv")
            {
                ...
                while ((moveText = info.section(' ', section, section, QString::SectionSkipEmpty)) != "")
                {
                    ...
                    auto move = board->moveFromString(moveText);
                    //if (!move.isLegal())
                    if (move.isNull())
                    {
                        break;
